  ...
  public function index()
{
    ...
    {
        // Find the childcategory
        $childcategory = Childcategory::where(['slug' => $slug, 'status' => 1])->firstOrFail();
    
        // Get sibling childcategories (for sidebar/filter etc.)
        $childcategories = Childcategory::where('subcategory_id', $childcategory->subcategory_id)
            ->where('status', 1)
            ->get();
    
        // Get products that belong to this childcategory
        $products = Product::where('status', 1)
            ->where('childcategory_id', $childcategory->id)
            ->select('id', 'name', 'slug', 'new_price', 'old_price', 'type', 'childcategory_id')
            ->withCount('variable');
    
        // Sorting
        switch ($request->sort) {
            case 1: $products = $products->orderBy('created_at', 'desc'); break; // Newest
            case 2: $products = $products->orderBy('created_at', 'asc'); break;  // Oldest
            case 3: $products = $products->orderBy('new_price', 'desc'); break;  // Price high → low
            case 4: $products = $products->orderBy('new_price', 'asc'); break;   // Price low → high
            case 5: $products = $products->orderBy('name', 'asc'); break;        // A → Z
            case 6: $products = $products->orderBy('name', 'desc'); break;       // Z → A
        ...
        $max_price = $products->max('new_price');
        if ($request->min_price && $request->max_price) {
            $products = $products->whereBetween('new_price', [$request->min_price, $request->max_price]);
        }
